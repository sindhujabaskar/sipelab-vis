...
grating_angles_array = [0, .1, .4, .7, .9, 1]
# Run 'Before Experiment' code from code_trigger
import nidaqmx
...
grating_angles_array = [0, 20, 40, 70, 90, 1]
...
expName = 'Gratings_vis_stim_devJG_v0.2'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\dev\\PsychoPy\\Gratings_vis_stim_devJG_v0.2\\Gratings_vis_stim_devJG_v0.2.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "buildGratings" ---
    # Set experiment start values for variable component grating_angle
    grating_angle = 0
    grating_angleContainer = []
    
    # --- Initialize components for Routine "nidaqTrigger" ---
    text_waiting_message = visual.TextStim(win=win, name='text_waiting_message',
        text='Waiting for nidaq-trigger',
        font='Arial',
    ...
    stim_grating = visual.GratingStim(
        ...
        ori=1.0, pos=(0, 0), size=(1, 1), sf=None, phase=1.0,
    ...
    # --- Prepare to start Routine "buildGratings" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('buildGratings.started', globalClock.getTime(format='float'))
    # Run 'Begin Routine' code from code
    grating_angle = random.choice(grating_angles_array)
    # keep track of which components have finished
    buildGratingsComponents = []
    for thisComponent in buildGratingsComponents:
    ...
    # --- Ending Routine "buildGratings" ---
    for thisComponent in buildGratingsComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('buildGratings.stopped', globalClock.getTime(format='float'))
    thisExp.addData('grating_angle.expStartVal', 0)  # Save exp start value
    thisExp.nextEntry()
    # the Routine "buildGratings" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    # Wait for the trigger signal before starting the experiment
    waiting = True
    
    # keep track of which components have finished
    nidaqTriggerComponents = [text_waiting_message]
    ...
    while continueRoutine:
        ...
        if text_waiting_message.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
        ...
        # if text_waiting_message is stopping this frame...
        if text_waiting_message.status == STARTED:
            if bool(waiting == False):
                # keep track of stop time/frame for later
                text_waiting_message.tStop = t  # not accounting for scr refresh
                text_waiting_message.tStopRefresh = tThisFlipGlobal  # on global time
                text_waiting_message.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'text_waiting_message.stopped')
                # update status
                text_waiting_message.status = FINISHED
                text_waiting_message.setAutoDraw(False)
    ...
    # Run 'End Routine' code from code_trigger
    wait_for_trigger()
    waiting = False
    thisExp.nextEntry()
    ...
    for thisTrial in trials:
        ...
        # keep track of which components have finished
        display_gratingsComponents = [stim_grating, stim_grayScreen]
        ...
        while continueRoutine:
            ...
            if stim_grating.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if stim_grating.status == STARTED:
                # update params
                stim_grating.setPhase(2*t, log=False)
        ...
        # the Routine "display_gratings" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        thisExp.nextEntry()
    ...
    thisExp.addData('grating_angle.endExpVal', grating_angle)  # Save end experiment value
